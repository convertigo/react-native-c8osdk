...
export class C8o {
    ...
    public callJson(requestable: string, parameters: any): C8oPromise<any> {
        ...
        // Add a new Listener  for the progress and the response for android
        if (this.ios) {
            ...
            this.subscription['response-' + uniqueID] = this._c8oManagerEmitter.addListener('response-' + uniqueID, (response) => {
                if (autoCancel) {
                    this.subscription[uniqueID].remove();
                    delete this.subscription[uniqueID];
                    this.subscription['response-' + uniqueID].remove();
                    delete this.subscription['response-' + uniqueID];
                }
                // resolve the response on the c8oPromise
                promise.onResponse(response, 'progress-' + uniqueID);
            });
